...
function removeRow() {
    const rowCount = table.rows.length;
    if (rowCount > MIN_ROWS) table.deleteRow(rowCount - 1);
    updateButtons();
}
function addColumn() {
    const rowCount = table.rows.length;
    const columnCount = table.rows[0].cells.length;
    if (columnCount < MAX_COLUMNS) for(let i = 0; i < rowCount; i++){
        const newCell = document.createElement("td");
...
function removeColumn() {
    const rowCount = table.rows.length;
    const columnCount = table.rows[0].cells.length;
    if (columnCount > MIN_COLUMNS) for(let i = 0; i < rowCount; i++)table.rows[i].deleteCell(columnCount - 1);
    updateButtons();
}
